...
const protobufDefs = {
  blueprint(pkgInfo) {
    ...
    const service = calls.length ? `service ${utils.caps(pkgInfo.pkg)} {
      ${calls.map(call => `  rpc ${call.get || call.set}(${call.input || noop}) ${protobufOutput(call, defns, output)}\n`).join('')}}`
      : '';

    const suffix = output.map(protobufDefinition).join('');
