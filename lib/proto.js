...
function protobufOutput(value, defns, output, modelId) {
  const suffix = modelId === value.resp ? '_' : '';
  const schema = `${value.resp}${suffix}`;
...
function protobufInput(value, params) {
  return `\nmessage ${value.input}${params ? '_' : ''} { string guid = 1; ${value.input} params = 2; }`;
}
...
function protobufCallDefinition(call, noop, defns, params, output, modelId) {
  let target = call.input || call.params;

  target = (target && `${target}${params ? '_' : ''}`) || noop;

  return `  rpc ${call.get || call.set}(${target}) ${protobufOutput(call, defns, output, modelId)}\n`;
}
...
const protobufDefs = {
  ...
  definition(pkgInfo, modelInfo) {
    const suffix = modelInfo.name === pkgInfo.id ? '_' : '';
    const output = [];

    output.push(utils.trim(`
      message ${modelInfo.name}${suffix} {${modelInfo.props ? (modelInfo.props.length ? '\n' : '')
