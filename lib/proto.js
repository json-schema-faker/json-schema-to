...
function protobufRequest(value, noop, params) {
  return value.input ? `${value.input}${params ? '_' : ''}` : noop;
}
...
function protobufCallDefinition(call, noop, defns, params, output) {
  return `  rpc ${call.get || call.set}(${protobufRequest(call, noop, params)}) ${protobufOutput(call, defns, output)}\n`;
}
