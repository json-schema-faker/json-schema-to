...
function protobufInput(value) {
  return `\nmessage ${value.input}_ { ${value.input} params = 1; }`;
}

function protobufRequest(value, noop) {
  return value.input ? `${value.input}_` : noop;
}
...
function protobufCallDefinition(call, noop, defns, output) {
  return `  rpc ${call.get || call.set}(${protobufRequest(call, noop)}) ${protobufOutput(call, defns, output)}\n`;
}
...
const protobufDefs = {
  blueprint(pkgInfo) {
    ...
    const service = calls.length ? `service ${utils.caps(pkgInfo.id || pkgInfo.pkg, true)} {
      ${calls.map(call => protobufCallDefinition(call, noop, defns, output)).join('')}}`
      : '';
