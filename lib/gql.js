...
function scanForNestedInput(ref, repo, model, inputs) {
  if (inputs.includes(ref) && Array.isArray(model.schema)) {
    model.schema.forEach(prop => {
      if (
        !prop.scalar
        && prop.exists
        && repo.models[prop.value]
        && !inputs.includes(prop.value)
      ) throw new TypeError(`'${prop.value}' cannot be used as input for '${ref}.${prop.field}'`);
      if (!prop.schema && !inputs.includes(prop.value)) inputs.push(prop.value);
    });
  }
}
...
module.exports = (repo, factory) => {
  ...
  inputs.forEach(ref => {
    scanForNestedInput(ref, repo, factory(repo.models[ref]), inputs);
  });
