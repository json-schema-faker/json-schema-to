  ...
  if (schema.properties) {
    ...
    Object.keys(schema.properties).forEach(prop => {
      ...
      if (value.items) {
        ...
        const schemaId = value.items.type || value.items.id || asType;

        if (!schemaId) {
          throw new Error(`Missing relationship, given ${_util.inspect(value.items)}`);
        }

        if (asType) {
          givenTypes.deps[asType] = givenTypes.deps[asType] || [];
          givenTypes.deps[asType].push(value.items.id || value.items.$ref);
        }
...
function generate(info, input, generator, definitions) {
  ...
  info.calls.forEach(callInfo => {
    if (callInfo.input && !callInfo.set) {
    ...
    ['req', 'resp', 'input'].forEach(param => {
      if (callInfo[param] && keys.indexOf(callInfo[param]) === -1) {
  ...
  input.models.forEach(modelInfo => {
    ...
    if (input.deps[modelInfo.name]) {
      const deps = input.deps[modelInfo.name];

      deps.splice(0, deps.length).forEach(prop => {
