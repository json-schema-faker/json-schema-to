  ...
  if (schema.properties) {
    ...
    Object.keys(schema.properties).forEach(prop => {
      ...
      if (value.id) {
        ...
        if (!givenTypes.models[value.id] && givenTypes.deps.indexOf(value.id) === -1) {
          givenTypes.deps.push(value.id);
          walk(value, givenTypes);
        }
        return;
      }
  ...
  info.calls.forEach(callInfo => {
    ...
    if (callInfo.input) {
      if (typeof callInfo.input === 'object') {
        Object.keys(callInfo.input).forEach(k => inputs.push(callInfo.input[k]));
      } else {
...
function conciliate(pkgInfo, definitions) {
  ...
  pkgInfo.calls.forEach(x => {
    ...
    if (typeof x.input === 'object') {
      Object.keys(x.input).forEach(k => push(x.input[k]));
    } else {
      push(x.input);
    }
  });
