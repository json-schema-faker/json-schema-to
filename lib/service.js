...
class Service {
  ...
  static bundle(pkgInfo, models) {
    ...
    models.forEach(_jst => {
      if (_jst instanceof Service) {
        _jst.enums.splice(0, _jst.enums.length).forEach(enumInfo => {
          const key = utils.uniq(enumInfo.values, map);
        ...
        _jst.models.forEach(modelInfo => {
          if (seen.indexOf(modelInfo.name) !== -1) {
            delete _jst.models[modelInfo.name];
            output[modelInfo.name].refs.push(utils.kebab(_jst.modelId));
            return;
          }
    ...
    return {
      ...
      get graphql() {
        return jst.generate({ pkg: 'x', calls: [] }, { enums }, jst.graphqlDefs, {});
      },

      get protobuf() {
        return jst.generate({ pkg: 'x', calls: [] }, { enums }, jst.protobufDefs, {});
      },
    };
  }
