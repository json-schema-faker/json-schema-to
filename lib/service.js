...
class Service {
  ...
  static bundle(pkgInfo, models) {
    ...
    const params = pkgInfo.params || undefined;
    ...
    models.forEach(_jst => {
      if (_jst instanceof Service) {
        ...
        _jst._resource.params = _jst._resource.params || params;

        if (pkgInfo.common) {
          _jst._resource.refs.push(pkgInfo.common);
        }
    ...
    return {
      ...
      get graphql() {
        return jst.generate({ pkg, calls: [] }, { enums }, jst.graphqlDefs, {});
      },

      get protobuf() {
        return jst.generate({
          pkg, refs, params, calls: [],
        }, { enums }, jst.protobufDefs, {});
      },
    };
  }
  ...
  static merge(pkgInfo, models) {
    ...
    const resource = {
      ...
      refs: pkgInfo.refs || [],
      calls: pkgInfo.calls || [],
      params: pkgInfo.params || undefined,
    };
    ...
    return Promise.resolve()
      .then(() => {
        Object.keys(_defns).forEach(def => {
          ...
          let fixedId = (items && (items.$ref || items.id)) || ref.$ref || ref.id || def;

          fixedId = fixedId.replace(/^#\/definitions\//, '');

          if (def !== fixedId) {
            this._resource.defns[def] = fixedId;
          }
