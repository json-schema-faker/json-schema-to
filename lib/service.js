...
class Service {
  constructor(resourceInfo) {
    ...
    Object.defineProperty(this, '_resource', {
      ...
      value: Object.assign(serviceDefinition, {
        defns: _defns,
        pkg: resourceInfo.pkg || schema.id,
        refs: resourceInfo.refs || [],
  ...
  load(directory, refs) {
    return jst.resolve(directory, refs || [], this._schema)
      .then(fixedSchema => jst.load(refs, fixedSchema, this._definitions))
      .then(() => this);
  }

  scan(directory, refs) {
    return jst.parse(directory, refs || [], this._schema, this._definitions)
      .then(() => this);
  }
